...
# to be placed into arch-i386/syscalls, as well as the content
# of arch-i386/linux/_syscalls.h
#
...
bionic_temp = "/tmp/bionic_gensyscalls/lib/"

# all architectures, update as you see fit
all_archs = [ "arm", "i386", "sh", "sparc64", "powerpc" ]
...
x86_return = """    ret
"""
# sparc64 assembler templates for each syscall stub
#

sparc64_header = """/* autogenerated by gensyscalls.py */
#include <sys/stos-syscalls.h>
...
# powerpc assembler templates for each syscall stub
#
...
class State:
    ...
    def sparc64_genstub(self, fname, flags, idname):
        t = { "fname"  : fname,
              "idname" : idname }
        result = sparc64_header % t
        result += sparc64_call % t
	result += sparc64_return % t
	return result
    ...
    def process_file(self,input):
        ...
        for t in self.syscalls:
            ...
            # Have a look at bionic_utils.py:203
            if t["id"] >= 0:
            ...
            if t["id2"] >= 0:
                ...
                if t["cid"] >= 0:
                    t["asm-i386"] = self.x86_genstub_cid(syscall_func, num_regs, "__NR_"+syscall_name, t["cid"])
                else:
                    t["asm-i386"] = self.x86_genstub(syscall_func, num_regs, "__NR_"+syscall_name)
            elif t["cid"] >= 0:
	    ...
	    if t["id4"] >= 0:
                num_regs = count_generic_param_registers(syscall_params)
                t["asm-sparc64"] = self.sparc64_genstub(syscall_func,num_regs,"__NR_"+syscall_name)
	    if t["id5"] >= 0:
                num_regs = count_generic_param_registers(syscall_params)
                t["asm-powerpc"] = self.powerpc_genstub(syscall_func,num_regs,"__NR_"+syscall_name)
    ...
    def gen_linux_syscalls_h(self):
        path = "../../include/sys/stos-syscalls.h"
        D( "generating "+path )
        fp = create_file( path )
        fp.write( "/* auto-generated by gensyscalls.py, do not touch */\n" )
        fp.write( "#ifndef _BIONIC_STOS_SYSCALLS_H_\n\n" )
    ...
    def gen_linux_unistd_h(self):
        path = "../../include/sys/stos-unistd.h"
        D( "generating "+path )
        fp = create_file( path )
        fp.write( "/* auto-generated by gensyscalls.py, do not touch */\n" )
        fp.write( "#ifndef _BIONIC_STOS_UNISTD_H_\n\n" );
    ...
    def gen_arch_syscalls_mk(self, arch):
        path = "arch-%s/syscalls/syscalls.mk" % arch
        D( "generating "+path )
        fp = create_file( path )
        fp.write( "# auto-generated by gensyscalls.py, do not touch\n" )
        fp.write( "OBJS := \n" )
        arch_test = {
            ...
            "i386": lambda x: x.has_key("asm-i386"),
            "sh": lambda x: x.has_key("asm-sh"),
            "sparc64": lambda x: x.has_key("asm-sparc64"),
    ...
    def gen_syscall_stubs(self):
        for sc in self.syscalls:
            ...
            if sc.has_key("asm-sparc64") and 'sparc64' in all_archs:
                fname = "arch-sparc64/syscalls/%s.S" % sc["func"]
                D( ">>> generating "+fname )
                fp = create_file( fname )
                fp.write(sc["asm-sparc64"])
                fp.close()
                self.new_stubs.append( fname )

            if sc.has_key("asm-powerpc") and 'powerpc' in all_archs:
            ...
            if sc.has_key("asm-i386") and 'i386' in all_archs:
                fname = "arch-i386/syscalls/%s.S" % sc["func"]
                D( ">>> generating "+fname )
                fp = create_file( fname )
                fp.write(sc["asm-i386"])
                fp.close()
                self.new_stubs.append( fname )
